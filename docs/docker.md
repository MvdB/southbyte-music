...
Tags: `0.1.4` and `0.1` come from git tags, `main` follows the default branch,
...
|---|---|---|
| `0.1.4`, `sha-<short>` | never | a run whose result has to stay attributable |
...
```bash
docker buildx imagetools inspect ghcr.io/mvdb/southbyte-music:0.1.4   # both platforms
```
...
| Variable | Default | |
|---|---|---|
| `SOUTHBYTE_MUSIC_VERSION` | `0.1.4` | tag for both images |
...
going through the proxy. That is why the pinned tag is `0.1.4`. See
[`oberflaeche.md`](oberflaeche.md).

**Two different things are both called 0.1.3, and they are unrelated.** This
repository's release `0.1.3` was tagged on 2026-08-16 from a tree that still
pinned sglang-omni `68abc7ee`; the pin to **SGLang-Omni v0.1.3** landed five days
later. Image tag `0.1.3` therefore still returns **mono** MP3, FLAC, Opus and AAC.
The first release whose server image carries the stereo fix is **`0.1.4`**. When
a version number appears below, check which of the two it means — the serving
stack or this repository.
...
IMAGE=ghcr.io/mvdb/southbyte-music:0.1.4 ./run_music.sh        # the published one
```
...
serving/pruefe_image.sh ghcr.io/mvdb/southbyte-music:0.1.4  # the published one
```
